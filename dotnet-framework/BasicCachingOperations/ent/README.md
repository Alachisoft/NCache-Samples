...
This sample is a .NET Framework 4.8.1 console application which demonstrates the use of NCache synchronous APIs to perform CRUD operations in the cache. It shows how to connect to a running cache and use Add, Get, Insert and Remove operations from NCache. At the end, the cache handler will be disposed.
...
cd "<path-to-sample>"
nuget restore BasicCachingOperations.sln
msbuild BasicCachingOperations.sln /p:Configuration=Debug
BasicCachingOperations\bin\Debug\BasicCachingOperations.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore BasicCachingOperations.sln` in the sample folder (see Build and Run section).
