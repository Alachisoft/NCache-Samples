...
cd "<path-to-sample>"
nuget restore SearchUsingSQL.sln
msbuild SearchUsingSQL.sln /p:Configuration=Debug
```
...
1. Build SampleData
    - Build the solution so `SampleData.dll` is produced in: `SampleData\bin\Debug\SampleData.dll` (or release folder if built in Release).
...
```bash
SearchUsingSQL\bin\Debug\SearchUsingSQL.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore SearchUsingSQL.sln` in the sample folder (see Build and Run section).
