...
cd "<path-to-sample>"
nuget restore BulkOperations.sln
msbuild BulkOperations.sln /p:Configuration=Debug
BulkOperations\bin\Debug\BulkOperations.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore BulkOperations.sln` in the sample folder (see Build and Run section).
