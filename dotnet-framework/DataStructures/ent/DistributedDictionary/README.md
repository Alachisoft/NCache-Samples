...
This .NET Framework 4.8.1 console sample demonstrates the NCache Distributed Dictionary data structure using demo data. A Distributed Dictionary is a cluster‑wide, thread‑safe key/value map that lets you store, retrieve, update and remove entries atomically from any client in the cache cluster.

The sample shows how to create or retrieve two distributed dictionaries for top selling and new arrival products. The workflow shows adding to both dictionaries, updating Product entries, fetching single entry by key, enumerating all items, removing entry by key and deleting the dictionaries from cache.
...
# e.g. d:\ncache-samples\DataStructures\DistributedDictionary
cd "<path-to-sample>"
nuget restore DistributedDictionary.sln
msbuild DistributedDictionary.sln /p:Configuration=Debug
DistributedDictionary\bin\Debug\DistributedDictionary.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore DistributedDictionary.sln` in the sample folder (see Build and Run section).
