...
This .NET Framework 4.8.1 console sample demonstrates the NCache Distributed Counter data structure. A Distributed Counter is a cluster‑wide, atomic numeric counter you can increment, decrement, read and reset from any client — operations are performed atomically across the cache cluster. 
...
cd "<path-to-sample>"
nuget restore DistributedCounter.sln
msbuild DistributedCounter.sln /p:Configuration=Debug
DistributedDictionary\bin\Debug\DistributedCounter.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore DistributedCounter.sln` in the sample folder (see Build and Run section).
