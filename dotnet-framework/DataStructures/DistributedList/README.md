...
# e.g. d:\ncache-samples\DataStructures\DistributedList
cd "<path-to-sample>"
nuget restore DistributedList.sln
msbuild DistributedList.sln /p:Configuration=Debug
DistributedList\bin\Debug\DistributedList.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore DistributedList.sln` in the sample folder (see Build and Run section).
