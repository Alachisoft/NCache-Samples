...
This .NET Framework 4.8.1 console application demonstrates how to use LINQ Queries with NCache. It shows how cached objects can be queried directly using standard LINQ syntax, allowing developers to run powerful, in-memory distributed queries against data stored inside NCache.
...
# e.g. d:\ncache-samples\NCacheLINQ
cd "<path-to-sample>"
nuget restore NCacheLINQ.sln
msbuild NCacheLINQ.sln /p:Configuration=Debug
```
...
```bash
cd "<path-to-sample>\NCacheLINQ\bin\Debug"
NCacheLINQ.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore NCacheLINQ.sln` in the sample folder (see Build section).
