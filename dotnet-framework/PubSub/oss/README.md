...
This .NET Framework 4.8.1 console sample demonstrates NCache Publish/Subscribe (Pub/Sub) which lets apps broadcast typed messages to named topics and lets consumers subscribe (non‑durable) to receive them. Use Pub/Sub for real‑time updates, notifications, dashboards, or decoupled event flows.

There are two .NET Framework 4.8.1 console application projects in this sample:

### Publisher

-	Demonstrates one publishing modes: synchronous (Publish).
	- Ensures topics (ElectronicsOrders, GarmentsOrders) exist or creates them.
	- Creates tasks to publishes messages.
...
- 	Demonstrates non‑durable subscriptions for topics that receive messages only while connected.
...
cd "<path-to-sample>"
nuget restore PubSub.sln
msbuild PubSub.sln /p:Configuration=Debug
```

### Run the Sample

You need to run the projects `NonDurableSubscriber` first and then run the `Publisher` project.
...
# e.g. d:\ncache-samples\PubSub
# Run project
cd "<path-to-sample>"\NonDurableSubscriber\bin\Debug'NonDurableSubscriber.exe
dotnet run
# Wait for the services to start and then run the publisher
cd "<path-to-sample>"\Publisher\bin\Debug'Publisher.exe
dotnet run
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore PubSub.sln` in the sample folder (see Build and Run section).
