...
This sample is a .NET Framework 4.8.1 console application which demonstrates NCache cache item locking. This is a locking mechanism to prevent concurrent modifications and preserve data consistency in a distributed cache. Using demo Customer objects, it shows thread-safe read‑modify‑write patterns and both implicit (GET with acquireLock) and explicit (Lock / Unlock) lock management, plus a simple add/remove lifecycle for the cached item.
...
cd "<path-to-sample>"
nuget restore CacheItemLocking.sln
msbuild CacheItemLocking.sln /p:Configuration=Debug
CacheItemLocking\bin\Debug\CacheItemLocking.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore CacheItemLocking.sln` in the sample folder (see Build and Run section).

## References

Reference documentation is available at:\
https://www.alachisoft.com/resources/docs/ncache/prog-guide/locking.html
