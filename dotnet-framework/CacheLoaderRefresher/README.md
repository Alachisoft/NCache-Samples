...
cd "<path-to-sample>"
nuget restore CacheLoaderRefresher.sln
msbuild CacheLoaderRefresher.sln /p:Configuration=Debug
```
...
1. Build the provider assembly
    - Build the solution so `LoaderRefresherLib.dll` is produced in: `LoaderRefresherLib\bin\Debug\LoaderRefresherLib.dll` (or release folder if built in Release).
...
```bash
LoaderRefresherUsage\bin\Debug\LoaderRefresherUsage.exe
```
...
- If NuGet packages are not restored properly:
	...
	- Command line: run `nuget restore CacheLoaderRefresher.sln` in the sample folder (see Build and Run section).
