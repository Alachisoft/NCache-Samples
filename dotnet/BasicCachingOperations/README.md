...
This sample is a .NET 8.0 console application which demonstrates the use of NCache synchronous APIs to perform CRUD operations in the cache. It shows how to connect to a running cache and use `Add`, `Get`, `Insert` and `Remove` operations from NCache. At the end, the cache handler will be disposed.
...
- If NuGet packages are not restored properly:
    ...
    - If using nuget.exe: run `nuget restore BasicCachingOperations.sln`.
