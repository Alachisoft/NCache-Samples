...
## Introduction
NCache Distributed Queue allows multiple application instances to share and process queued data while preserving FIFO (First-In-First-Out) ordering.
This .NET 8.0 console sample demonstrates how to use the NCache Distributed Queue data structure to store and process `Customer` objects in a distributed, thread-safe FIFO (First-In-First-Out) queue. The sample covers creating or retrieving a queue with expiration, registering and unregistering enqueue/dequeue event callbacks, adding items, and performing common queue operations including peek, dequeue, display, clear, and cleanup.

This sample uses the SampleData project as a reference for the `Customer` model class.
...
# e.g. d:\ncache-samples\DistributedQueue
cd "<path-to-sample>"
dotnet restore
dotnet build
dotnet run
...
- If NuGet packages are not restored properly:
	...
	- **nuget.exe**: Run `nuget restore DistributedQueue.sln`.
...
For more information about Distributed Queue, see:\
