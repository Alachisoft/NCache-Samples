...
This sample is a Java (JDK 21) Maven console application which demonstrates the use of JCache APIs to perform CRUD operations and events (Cache-Level and Item-Level) in the cache. It shows how to connect to a running cache and use `put`, `get`, `remove`, `getAll`, `putAll` and `removeAll` operations from NCache along with how to receive events for some of those operations. At the end, the cache handler will be disposed.
...
https://www.alachisoft.com/resources/docs/ncache/prog-guide/jcache.html
